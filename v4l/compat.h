...
static char *devm_kasprintf(struct device *dev, gfp_t gfp, const char *fmt, ...)
{
	va_list ap;
	char *p;
...
static int _media_regmap_read(struct regmap *map, unsigned int reg,
			unsigned int *val)
{
	int ret;
...
static int _media_regmap_write(struct regmap *map, unsigned int reg,
		  unsigned int val)
{
	int ret;
	void *context = _regmap_map_get_context(map);
			       ...
			       bool *change)
{
	...
	ret = _media_regmap_read(map, reg, &orig);
	if (ret != 0)
		return ret;
	...
	if (tmp != orig) {
		ret = _media_regmap_write(map, reg, tmp);
		*change = true;
	} else {
		*change = false;
	}
...
static int regmap_write_bits(struct regmap *map, unsigned int reg,
		      unsigned int mask, unsigned int val)
{
	int ret;
